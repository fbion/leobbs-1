...
sub _parts {
  ...
  for my $value (@$values) {
    ...
    if (ref $value eq 'HASH') {
      ...
      $file_name = delete $value->{filename};
      $headers->from_hash($value);
      next unless defined $name;
      $file_name = url_escape $file_name // $name, '"';
      $file_name = encode $charset, $file_name if $charset;
    }
    ...
    $disposition .= qq{; filename="$file_name"} if defined $file_name;
