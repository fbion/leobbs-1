...
sub _parse_formdata {
  ...
  while (my $part = shift @parts) {
    ...
    my ($file_name) = $disposition =~ /[; ]filename="((?:\\"|[^"])*)"/;
    next if $upload && !defined $file_name || !$upload && defined $file_name;
    my ($name) = $disposition =~ /[; ]name="((?:\\"|[^;"])*)"/;
    $part = $part->asset->slurp unless $upload;

    if ($charset) {
      $name     = decode($charset, $name) // $name         if $name;
      $file_name = decode($charset, $file_name) // $file_name if $file_name;
      $part = decode($charset, $part) // $part unless $upload;
    }

    push @formdata, [$name, $part, $file_name];
  }
