   ...
   if ($$post =~ /\[UploadFile.{0,6}=.+?\]/) {
        ...
        if (($nodispphoto ne 'yes' && $arrawpostpic eq 'on') || $member_code{$membername} eq 'mo' || $member_code{$membername} eq 'amo' || $member_code{$membername} eq 'cmo' || $member_code{$membername} eq 'ad' || $inmembmod eq 'yes' || $member_code{$membername} eq 'smo') {
            if ($imgslt eq "Disp") {
            	...
            	while ($$post =~ /\[UploadFileDisp=([^\]]+?)\.(gif|jpg|png|bmp|jpeg)\]/i) {
            	    $jsq ++;
            	    if (($jsq eq $sltnoperline)&&($sltnoperline ne 0)) {
                        $$post =~ s/\[UploadFileDisp=([^\]]+?)\.(gif|jpg|png|bmp|jpeg)\]/<a href=$imagesurl\/$usrdir\/$in_forum\/$tmptopic\/$1\.$2 target=_blank><img src=$imagesurl\/$usrdir\/$in_forum\/$tmptopic\/$1\.$2 border=0 alt=按此在新窗口浏览图片 width=$defaultsltwidth height=$defaultsltheight onmousewheel="return bbimg(this)"><\/a><BR><BR>/i;
                        $jsq = 0;
                    } else {
                        $$post =~ s/\[UploadFileDisp=([^\]]+?)\.(gif|jpg|png|bmp|jpeg)\]/<a href=$imagesurl\/$usrdir\/$in_forum\/$tmptopic\/$1\.$2 target=_blank><img src=$imagesurl\/$usrdir\/$in_forum\/$tmptopic\/$1\.$2 border=0 alt=按此在新窗口浏览图片 width=$defaultsltwidth height=$defaultsltheight onmousewheel="return bbimg(this)"><\/a>　/i;
                    }
                }
            } else {
                $$post =~ s/\[UploadFileDisp=([^\]]+?)\.(gif|jpg|png|bmp|jpeg)\]/<a href=$imagesurl\/$usrdir\/$in_forum\/$tmptopic\/$1\.$2 target=_blank><img src=$imagesurl\/$usrdir\/$in_forum\/$tmptopic\/$1\.$2 border=0 alt=按此在新窗口浏览图片 onload="javascript:if(this.width>document.body.clientWidth-333)this.width=document.body.clientWidth-333" onmousewheel="return bbimg(this)"><\/a>/isg;
            }
            if ($thisprog eq "post.cgi") {
	...
	while ($$post =~ /\[UploadFile.{0,6}=([^\\\]]+?)(\.(.*?))\]/i) {
   	    $file_type = (-e "${imagesdir}icon/$3.gif") ? $3 : "unknow";
            $$post =~ s/\[UploadFile.{0,6}=([^\\\]]+?)(\.(.*?))\]/<br><img src=$imagesurl\/icon\/$file_type.gif border=0 width=16> 这个是上传的 $3 格式文件 [<a href=attachment.cgi?forum=$in_forum&topic=$in_topic&postno=$editpostnumber&name=$1&type=$2 target=_blank><B>点击查看<\/b><\/a>]<br>/isg;
	}
    }
    return;
}
