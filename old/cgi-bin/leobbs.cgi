...
if (((-e "${lbdir}cache/forumcache-$skin.pl") && ((-M "${lbdir}cache/forumcache-$skin.pl") * 86400 < 220)) || (-e "${lbdir}cache/forumcache-$skin.pl.lock")) {
    ...
    if (-e "${lbdir}cache/forumcache-$skin.pl") {
        ...
        if ($@) {
            unlink("${lbdir}cache/forumcache-$skin.pl");
            require "doforumcache.pl";
        }
        $forumcached = "yes";
    }
...
if ($action eq "union") {if ($union == 1) {$union = 0;}
else {$union = 1;}}
...
~;
    ...
    if ($in_member_name eq "客人") {
        ...
        if ((-e "${lbdir}cache/myinfo/$memberfilename.pl") && (((-M "${lbdir}cache/myinfo/$memberfilename.pl") * 86400 < 1800) || ($docache eq "yes"))) {
            ...
            if ($@) {
                unlink("${lbdir}cache/myinfo/$memberfilename.pl");
                require "domyinfo.pl";
            }
        }
...
if (($createyear > 0) && ($createmon > 0) && ($createday > 0)) {$createday = "自 <font color=$fontcolormisc2>$createyear年$createmon月$createday日</font> 创建以来，";}
else {$createday = "";}
