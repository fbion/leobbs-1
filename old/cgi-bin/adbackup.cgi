...
if ($memdir ne "") {$memberdir = $memdir;}
elsif ($memberdir eq "") {$memberdir = "members";}
...
sub dorestore {
    ...
    chdir $lbdir;
    if ($oldversion eq "yes") {&oldrestore;}
    else #new restore
    {
        $toopen = @dirtoopen[0];
            ...
            if ($tarfile[0] eq "tarfilelist.txt") {
                ...
                if ($havetopic > 0) {
                    ...
                    for ($i = 13; $i < $#tarinfo; $i++) {
                        ...
                        if ($tarinfo[$i] =~ m/forum/) {push(@forumlist, $tarinfo[$i]);}
                    }
                    chomp(@forumlist);
       ...
       <input type=hidden name="action" value="untar">
        ...
        if (-e "tarfileindex.txt") {
            ...
            foreach (@needtotar) {
                ...
                if (-e "$isok") {$status .= "$isok 分包成功<br>";}
                else {
                    $status .= "$isok 分包<font color=blue>未成功</font>。请检查是否使用二进制模式上传！<br>";
                    $error = "yes";
                }
                if (opendir(DIR, "$dirname")) {$status .= "$dirname 存在<br>";}
                else {
                    $status .= "$dirname <font color=blue>不存在</font>";
                    if (mkdir("$dirname", 777)) {
                        chmod(0777, "$dirname");
                        $status .= "&nbsp 目录创建成功<br>";
                    }
                    else {
                        $status .= "&nbsp目录创建失败<br>";
                        $error = "yes";
                    }
                } #else
...
~;
    ...
    if ($#memberdata < 0) {$cc = 0;}
    else {$cc = $#memberdata + 1;}
    if ($#systemdata < 0) {$dd = 0;}
    else {$dd = $#systemdata + 1;}
    if ($#avatardata < 0) {$ee = 0;}
    else {$ee = $#avatardata + 1;}
    if ($#msgdatain < 0) {$ff = 0;}
    else {$ff = $#msgdatain + 1;}
    if ($#msgdataout < 0) {$gg = 0;}
    else {$gg = $#msgdataout + 1;}
