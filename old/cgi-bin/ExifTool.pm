...
sub GifInfo($) {
    ...
    if (read($GIF, $s, 3) == 3) {
        if (ord($s) & 0x80) {
            # does this image contain a color table?
...
sub FormattedValue($$$$) {
    ...
    for (my $i = 0; $i < $count; ++$i) {
        my $val;
        if ($format == 1 or ($format == 7 and $count == 1)) {
            # unsigned byte or single unknown byte
