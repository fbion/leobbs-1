...
foreach (@params) {
    ...
    if ($_ eq 'createyear') {
        $theparam = sprintf("%02d", $theparam);
        $theparam = 1900 + $theparam if ($theparam < 100);
    }
...
if (($member_code eq "ad") && ($in_password eq $password) && ($password ne "") && ($in_member_name ne "") && (lc($in_member_name) eq lc($membername))) {
    ...
    if ($action eq "process") {
        ...
        print qq~
                ...
                <tr>
                <td bgcolor=#FFFFFF width=40%>
                <font color=#333333><b>论坛首页音乐名称</b>(如果没有请留空)<br>请输入背景音乐名称，背景音乐<BR>应上传于 assets/midi 目录下。<br><b>不要包含 URL 地址或绝对路径！</b></font></td>
                <td bgcolor=#FFFFFF>
