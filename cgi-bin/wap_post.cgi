...
$|++;
&waptitle;
$show.= qq~<card  title="$boardname-ÂèëË°®‰∏ªÈ¢ò">~;
$lid = $query -> param('lid');
&check($lid);
$inforum        = $query -> param('f');
$intopictitle        = $query -> param('title');
if ($inmembername eq "" || $inmembername eq "ÂÆ¢‰∫∫" ) {
    $inmembername = "ÂÆ¢‰∫∫";
} else {
...
if ($allowusers ne ''){
    &errorout('ËøõÂÖ•ËÆ∫Âùõ&‰Ω†‰∏çÂÖÅËÆ∏ËøõÂÖ•ËØ•ËÆ∫ÂùõÔºÅ') if (",$allowusers," !~ /,$inmembername,/i && $membercode ne 'ad');
}
if ($membercode ne 'ad' && $membercode ne 'smo' && $inmembmod ne 'yes') {
    &errorout("ËøõÂÖ•ËÆ∫Âùõ&‰Ω†‰∏çÂÖÅËÆ∏ËøõÂÖ•ËØ•ËÆ∫ÂùõÔºå‰Ω†ÁöÑÂ®ÅÊúõ‰∏∫ $ratingÔºåËÄåÊú¨ËÆ∫ÂùõÂè™ÊúâÂ®ÅÊúõÂ§ß‰∫éÁ≠â‰∫é $enterminweiwang ÁöÑÊâçËÉΩËøõÂÖ•ÔºÅ") if ($enterminweiwang > 0 && $rating < $enterminweiwang);
    if ($enterminmony > 0 || $enterminjf > 0 ) {
	require "data/cityinfo.cgi" if ($addmoney eq "" || $replymoney eq "" || $moneyname eq "");
	$mymoney1 = $numberofposts * $addmoney + $numberofreplys * $replymoney + $visitno * $loginmoney + $mymoney - $postdel * $delmoney + $jhcount * $addjhhb;
	&errorout("ËøõÂÖ•ËÆ∫Âùõ&‰Ω†‰∏çÂÖÅËÆ∏ËøõÂÖ•ËØ•ËÆ∫ÂùõÔºå‰Ω†ÁöÑÈáëÈí±‰∏∫ $mymoney1ÔºåËÄåÊú¨ËÆ∫ÂùõÂè™ÊúâÈáëÈí±Â§ß‰∫éÁ≠â‰∫é $enterminmony ÁöÑÊâçËÉΩËøõÂÖ•ÔºÅ") if ($enterminmony > 0 && $mymoney1 < $enterminmony);
	&errorout("ËøõÂÖ•ËÆ∫Âùõ&‰Ω†‰∏çÂÖÅËÆ∏ËøõÂÖ•ËØ•ËÆ∫ÂùõÔºå‰Ω†ÁöÑÁßØÂàÜ‰∏∫ $jifenÔºåËÄåÊú¨ËÆ∫ÂùõÂè™ÊúâÁßØÂàÜÂ§ß‰∫éÁ≠â‰∫é $enterminjf ÁöÑÊâçËÉΩËøõÂÖ•ÔºÅ") if ($enterminjf > 0 && $jifen < $enterminjf);
    }
}
if ($startnewthreads eq "onlysub") {&errorout("ÂèëË°®&ÂØπ‰∏çËµ∑ÔºåËøôÈáåÊòØÁ∫ØÂ≠êËÆ∫ÂùõÂå∫Ôºå‰∏çÂÖÅËÆ∏ÂèëË®ÄÔºÅ"); }
	...
	if ($lastpost > $currenttime)  {
	    ...
	    &errorout("ÂèëË°®Êñ∞‰∏ªÈ¢ò&ÁÅåÊ∞¥È¢ÑÈò≤Êú∫Âà∂Â∑≤Áªè‰ΩøÁî®ÔºåÊÇ®ÂøÖÈ°ªÂÜçÁ≠âÂæÖ $lastpost1 ÁßíÈíüÊâçËÉΩÂÜçÊ¨°ÂèëË°®ÔºÅ");
	}
    }
    if ($postopen eq "no") { &errorout("ÂèëË°®ÊàñÂõûÂ§ç‰∏ªÈ¢ò&ÂØπ‰∏çËµ∑ÔºåÊú¨ËÆ∫Âùõ‰∏çÂÖÅËÆ∏ÂèëË°®ÊàñÂõûÂ§ç‰∏ªÈ¢òÔºÅ"); }
    if (($userregistered eq "no")&&(length($inmembername) > 12)) { &errorout("ÂèëË°®Êñ∞‰∏ªÈ¢ò&ÊÇ®ËæìÂÖ•ÁöÑÁî®Êà∑ÂêçÂ§™ÈïøÔºåËØ∑ÊéßÂà∂Âú®6‰∏™Ê±âÂ≠óÂÜÖÔºÅ");   }
    if (($userregistered eq "no")&&($inmembername =~ /^ÂÆ¢‰∫∫/)) { &errorout("ÂèëË°®Êñ∞‰∏ªÈ¢ò&ËØ∑‰∏çË¶ÅÂú®Áî®Êà∑ÂêçÁöÑÂºÄÂ§¥‰∏≠‰ΩøÁî®ÂÆ¢‰∫∫Â≠óÊ†∑ÔºÅ");   }
    if ($inmembername eq "ÂÆ¢‰∫∫") { &errorout("ÂèëË°®Êñ∞‰∏ªÈ¢ò&ËØ∑‰∏çË¶ÅÂú®Áî®Êà∑ÂêçÁöÑÂºÄÂ§¥‰∏≠‰ΩøÁî®ÂÆ¢‰∫∫Â≠óÊ†∑ÔºÅ");   }
    if (($userregistered eq "no")&&($startnewthreads ne "all")) { &errorout("ÂèëË°®Êñ∞‰∏ªÈ¢ò&ÊÇ®Ê≤°ÊúâÊ≥®ÂÜåÔºÅ");   }
   # elsif ((($inpassword ne $password)&&($userregistered ne "no"))||(($inpassword ne "")&&($userregistered eq "no"))) { &errorout("ÂèëË°®Êñ∞‰∏ªÈ¢ò&ÊÇ®ÁöÑÂØÜÁ†ÅÈîôËØØÔºÅ"); }
    elsif (($membercode eq "banned")||($membercode eq "masked"))      { &errorout("Ê∑ªÂä†ÂõûÂ§ç&ÊÇ®Ë¢´Á¶ÅÊ≠¢ÂèëË®ÄÊàñËÄÖÂèëË®ÄË¢´Â±èËîΩÔºåËØ∑ËÅîÁ≥ªÁÆ°ÁêÜÂëòËß£ÂÜ≥ÔºÅ"); }
    elsif ($intopictitle eq "ÔºäÔºÉÔºÅÔºÜÔºä") { &errorout("ÂèëË°®Êñ∞‰∏ªÈ¢ò&ÂøÖÈ°ªËæìÂÖ•‰∏ªÈ¢òÊ†áÈ¢òÔºÅ"); }
    elsif (length($intopictitle) > 92)   { &errorout("ÂèëË°®Êñ∞‰∏ªÈ¢ò&‰∏ªÈ¢òÊ†áÈ¢òËøáÈïøÔºÅ"); }
    else  {
	$intopictitle =~ s/\(Êó†ÂÜÖÂÆπ\)$//;
	if (($inpost eq "")&&($addme eq "")) { $intopictitle.=" (Êó†ÂÜÖÂÆπ)"; }}
        $intopictitle =~ s/()+//isg;
	my $tempintopictitle = $intopictitle;
	$tempintopictitle =~ s/ //g;
	$tempintopictitle =~ s/\&nbsp\;//g;
        $tempintopictitle =~ s/„ÄÄ//isg;
        $tempintopictitle =~ s/mo')||($inmembmod eq "yes")) { $allowed = "yes"; }
        if (($privateforum eq "yes") && ($allowed ne "yes")) { &errorout("∑¢±Ì&∂‘≤ª∆£¨ƒ˙≤ª‘ –Ì‘⁄¥À¬€Ã≥∑¢±Ì£°"); }

	if ($startnewthreads eq "no") {
            unless ($membercode eq "ad" ||$membercode eq 'smo'|| $inmembmod eq "yes") { &errorout("∑¢±Ì–¬÷˜Ã‚&‘⁄¥À¬€Ã≥÷–÷ªƒ‹”…Ã≥÷˜ªÚ’ﬂ±æ∞Ê∞Ê÷˜∑¢±Ì–¬÷˜Ã‚£°"); }
	}
	elsif ($startnewthreads eq "cert") {
            unless ($membercode eq "ad" ||$membercode eq 'smo'|| $inmembmod eq "yes"||$membercode eq 'cmo'||$membercode eq 'mo'||$membercode eq 'amo'||$membercode =~ /^rz/) { &errorout("∑¢±Ì–¬÷˜Ã‚&‘⁄¥À¬€Ã≥÷–÷ªƒ‹”…Ã≥÷˜°¢∞Ê÷˜∫Õ»œ÷§ª·‘±∑¢±Ì–¬÷˜Ã‚£°"); }
	}
	elsif (($startnewthreads eq "follow") &&($action eq "addnew")) {
            unless ($membercode eq "ad" ||$membercode eq 'smo'||$membercode eq 'cmo' ||$membercode eq 'mo'||$membercode eq 'amo'|| $inmembmod eq "yes") { &errorout("∑¢±Ì–¬÷˜Ã‚&‘⁄¥À¬€Ã≥÷–÷ªƒ‹”…Ã≥÷˜ªÚ’ﬂ∞Ê÷˜∑¢±Ì–¬÷˜Ã‚£°"); }
	}
	elsif (($startnewthreads eq "all")&&($userregistered eq "no")) { $inmembername = "$inmembername(øÕ)"; }
	...
	if ($deletepercent > 0 && $numberofposts + $numberofreplys > 0 && $membercode ne "ad" && $membercode ne "smo" && $membercode ne "cmo" && $membercode ne "mo" && $membercode ne "amo" && $inmembmod ne "yes") {
	    &errorout("∑¢±Ì–¬÷˜Ã‚&∂‘≤ª∆£¨ƒ„µƒ…æÃ˘¬ ≥¨π˝¡À<b>$deletepercent</b>%£¨π‹¿Ì‘±≤ª‘ –Ìƒ„∑¢±Ì–¬÷˜Ã‚£°«Î¡™œµÃ≥÷˜Ω‚æˆ£°") if ($postdel / ($numberofposts + $numberofreplys) >= $deletepercent / 100);
	}
        $inpost =~ s/\[’‚∏ˆ(.+?)◊Ó∫Û”…(.+?)±‡º≠\]//isg;
        	$inpost ="<img src='$imagesurl/images/sj.gif' width='22' alt='”… ÷ª˙ WAP ∑¢ÀÕ' /> ".$inpost;
	...
	my $oldthreadnumber = $newthreadnumber - 1;
	    ...
	    if (($amembername eq $inmembername)&&((($apost eq $inpost)&&($apost ne "")&&($inpost ne ""))||($atopictitle eq $intopictitle))) {
	    	...
	    	&errorout("∑¢±Ì–¬÷˜Ã‚&«Î≤ª“™÷ÿ∏¥∑¢Ã˚◊”£¨“—æ≠¥Ê‘⁄”Î¥ÀÃ˚◊”÷˜Ã‚œ‡Õ¨ªÚ’ﬂƒ⁄»›œ‡Õ¨µƒ∂¯«“ «ƒ„∑¢µƒÃ˚◊”¡À£°");
	    }
	}
		...
		if (-e $filetomakeopen) {
		    ...
		    if (($membercode ne "ad")&&($membercode ne "smo")&&($membercode ne "cmo")&&($membercode ne "mo")&&($membercode ne "amo")&&($membercode !~ /^rz/)) {
			    ...
			    if (($inmembername ne "")&&($userregistered ne "no")&&($password ne "")) {
		            ...
		            close (FILE);
			    &errorout("≥ˆ¥Ì&”…”⁄ƒ„‘⁄∂‡«¯∑¢ÀÕπ„∏Ê£¨À˘“‘ƒ„“—æ≠±ªΩ˚÷π∑¢—‘£°");
			}
		    }
...
unlink ("${lbdir}cache/plcache$inforum\_0.pl");
    $show.= qq~<p>–¬÷˜Ã‚∑¢±Ì≥…π¶..<br/><a href="wap_forum.cgi?forum=$inforum&amp;lid=$lid&amp;paGe=$pa">∑µªÿ¡–±Ì..</a></p><p><a href="wap_topic.cgi?f=$inforum&amp;lid=$lid&amp;t=$newthreadnumber">∑µªÿÃ˚◊”..</a></p>~;
&wapfoot;