...
sub deletepost {
        ...
        if ($cleartoedit eq "yes" && ($checkcandel == $delcount || $checkcandelm eq "YES")) {
            ...
            foreach $pno (@oldinpostno) {
       	   ...
       	   if($ #files >= 0){
                        ...
                        yes " && $DTHack=~m/$inforum\_$intopic\_$pno/);
       	   	foreach $file(@files){
       	   unlink ("
                        $dirtoopen2 / $file
                        ");
	...
	$filetoopen = "
                        ${lbdir}
                        boarddata / foruminfo $inforum . cgi ";
	my $filetoopens = &lockfilename($filetoopen);
	if (!(-e "
                        $filetoopens . lck ")) {
	    ...
	    open(FILE, ">
                        ${lbdir}
                        boarddata / forumposts $inforum . pl ");
	    print FILE "
                        \$threads = $threads;
                        \n \$posts = $posts;
                        \n \$todayforumpost = \"$todayforumpost\";\n1;\n";
...
sub unpostdeleteonce {
    &mischeader("取消单贴屏蔽");
  if ($checked eq "yes") { #1
    $filetoopen = "$lbdir" . "forum$inforum/
                                $intopic . thd . cgi ";
    if (-e $filetoopen) {
       &winlock($filetoopen) if ($OS_USED eq "
                                Nt ");
       ...
       &winunlock($filetoopen) if ($OS_USED eq "
                                ...
                                $lbdir
                                " . "
                                forum $inforum / $intopic . pl "); &error("
                                取消单贴屏蔽 & 这个主题不存在！"); }
    chomp @allthreads;
    $cleartoedit = "
                                no ";
