...
sub checksearchbot {
    #返回 0 = 搜索引擎访问 ，返回 1 = 非搜索引擎（或者不允许搜索引擎直接访问）
    ...
    if ($in_member_name eq "" || $in_member_name eq "客人") {
        ...
        if (($thisprog eq "leobbs.cgi") || ($thisprog eq "forums.cgi") || ($thisprog eq "topic.cgi")) {
            ...
            if ((-e "${lbdir}cache/mymsg/$memberfilename.pl") && ((-M "${lbdir}cache/mymsg/$memberfilename.pl") < (-M "${lbdir}$msgdir/in/${memberfilename}_msg.cgi"))) {
                ...
                if ($@) {
                    unlink("${lbdir}cache/mymsg/$memberfilename.pl");
                    $totalmessages = 0;
                    $unread = 0;
                }
            }
...
sub whosonline {
    ...
    foreach my $line (@onlinedata1) {
        ...
        if ($userexpire <= $savedtime) {
            if ($savedusername !~ /^客人/) {if ($savehidden ne 1) {$onlineuserlist = "$onlineuserlist$savedusername\_";}
            else {$onlineuserlisthidden = "$onlineuserlisthidden$savedusername\_";}}
...
~;
        foreach my $line (@onlinedata) {
            ...
            if ($tablewidth > 100) {
                if ($tablewidth > 1000) {$screenmodes = 10;}
                elsif ($tablewidth > 770) {$screenmodes = 8;}
                else {$screenmodes = 6;}
            }
            else {
