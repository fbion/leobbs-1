...
&error("$plugname 后台管理&只有论坛坛主与插件管理员才能进入此区！") if (($member_code ne "ad") && ($admin_user ne "$tempmembername"));
...
sub bset # 基本设置
{
    &error("基本设置&此选项只有本论坛坛主才能使用！") if ($member_code ne "ad");
...
sub edit_sort {
    ...
    if ($edit eq 'y') {
        ...
        for ($i = 0; $i < @sort; $i++) {
            ...
            else {print FILE "$cateid\t$old_id\t$old_status\t$old_name\t$old_info";}
        }
        close(FILE);
        $output .= qq~
<table cellPadding=6 cellSpacing=1 width=100%>
...
<TABLE border=0 cellPadding=0 cellSpacing=1 width=100% align=center>~;
        ...
        my $showpage = "";
        if (!$allpages) {$showpage .= "当前没有记录";}
        elsif ($allpages == 1) {$showpage .= "当前记录只有 <B>1</B> 页";}
        else {
