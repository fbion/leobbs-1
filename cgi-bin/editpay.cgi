...
sub editform {
    ...
    &error("å‘è¡¨&å¯¹ä¸èµ·ï¼Œè¿™ä¸ªä¸æ˜¯äº¤æ˜“å¸–ï¼") unless ($post=~m/\[ALIPAYE\]/);
...
if ($postage_mail ne "" || $postage_express ne "" || $postage_ems ne "") {
    ...
    $output.=qq~</tr>
<tr><td bgcolor=$miscbackone><font color=$fontcolormisc><b>ÎÒµÄÖ§¸¶±¦ÕËºÅ</b> <font color=$fonthighlight>(*)</font></font></td><td bgcolor=$miscbackone><input type=text size=28 maxlength=80 name="alipayid" value="$alipayid">¡¡Èç¹ûÃ»ÓĞ£¬ÇëÌîĞ´ÕıÈ·µÄÓÊ¼şµØÖ·</td></tr>~;
$output.=qq~</tr>
<tr><td bgcolor=$miscbackone><font color=$fontcolormisc><b>ÉÌÆ·Ãû³Æ</b> <font color=$fonthighlight>(*)</font></font></td><td bgcolor=$miscbackone><input type=text size=28 maxlength=80 name="warename" value="$warename">¡¡Ã»ÓĞÃû³Æ£¬Âò¼ÒÔõÃ´ÂòÄØ?</td></tr>~;
$output.=qq~</tr>
<tr><td bgcolor=$miscbackone><font color=$fontcolormisc><b>ÉÌÆ·Õ¹Ê¾µØÖ·</b></font></td><td bgcolor=$miscbackone><input type=text size=28 maxlength=80 name="wareurl" value="$wareurl">¡¡¸ø¿Í»§¸ü¼ÓÏêÏ¸µÄ½éÉÜ</td></tr>~;
    $output.=qq~</tr>
<tr><td bgcolor=$miscbackone><font color=$fontcolormisc><b>ÉÌÆ·¼Û¸ñ</b> <font color=$fonthighlight>(*)</font></font></td><td bgcolor=$miscbackone><input type=text size=28 maxlength=80 name="wareprice" value="$wareprice">¡¡ÇëÌîĞ´ÕıÈ·µÄ¼Û¸ñ</td></tr>~;
    $output.=qq~</tr>
<tr><td bgcolor=$miscbackone><font color=$fontcolormisc><b>ÓÊ·Ñ³Ğµ£·½Ñ¡Ôñ</b> <font color=$fonthighlight>(*)</font></font></td><td bgcolor=$miscbackone>
<input onclick="document.FORM.postage_mail.disabled=true; document.FORM.postage_express.disabled=true; document.FORM.postage_ems.disabled=true" type="radio" $tcheck1 value="s" name="transport"> Âô¼Ò³Ğµ£ÓÊ·Ñ<br>
<input onclick="document.FORM.postage_mail.disabled=false; document.FORM.postage_express.disabled=false; document.FORM.postage_ems.disabled=false" type="radio" $tcheck2 value="b" name="transport"> Âò¼Ò³Ğµ£ÓÊ·Ñ<br>
Í¨¹ıÎïÁ÷·ÑÓÃÑ¡Ôñ×¢Ã÷¸Ã½»Ò×ÊÇÓÉÂòÂôÄÄ·½³Ğµ£ÔË·Ñ¡£<br>
Èç¹ûÊÇÂò¼Ò³Ğµ£ÔË·Ñ£¬ÇëÑ¡Ôñ¿ÉÒÔÌá¹©µÄÎïÁ÷·½Ê½ÒÔ¼°ÏàÓ¦·ÑÓÃ¡£<br>
Æ½ÓÊ <input $pviewed size="3" name="postage_mail" value="$postage_mail"> Ôª (²»ÌîÊÓ×÷²»Ìá¹©Æ½ÓÊ)<br>
¿ìµİ <input $pviewed size="3" name="postage_express" value="$postage_express"> Ôª (²»ÌîÊÓ×÷²»Ìá¹©¿ìµİ)<br>
EMS&nbsp; <input $pviewed size="3" name="postage_ems" value="$postage_ems"> Ôª (²»ÌîÊÓ×÷²»Ìá¹© EMS)<br>
</td></tr>
~;
    $maxpoststr = "(Ìû×ÓÖĞ×î¶à°üº¬ <B>400</B> ¸ö×Ö·û)" ;

    $output .= qq~<td bgcolor=$miscbackone valign=top><font color=$fontcolormisc><b>ÉÌÆ·ÃèÊö</b> <font color=$fonthighlight>(*)</font>¡¡$maxpoststr<p>
...
<input type=checkbox name="inshowsignature" value="yes" checked>ÊÇ·ñÏÔÊ¾ÄúµÄÇ©Ãû£¿<br>$requestnotify$emoticonsbutton$fontpost$weiwangoptionbutton
...
$edittimes = 0 unless ($edittimes);
    ...
    &error("±à¼­Ìû×Ó&¶Ô²»Æğ£¬Õâ¸ö²»ÊÇ½»Ò×Ìû£¡") unless ($post=~m/\[ALIPAYE\]/);
