...
$userregistered	= "no";
&ERROROUT("ÀÏ´ó£¬±ðÂÒºÚÎÒµÄ³ÌÐòÑ½£¡") if (($inmembername =~  m/\//)||($inmembername =~ m/\\/)||($inmembername =~ m/\.\./));
unless ((!$inmembername) or ($inmembername eq "¿ÍÈË")) { 
	&getmember("$inmembername","no");
	$mymembercode=$membercode;
	$myrating=$rating;
	&ERROROUT("ÃÜÂëÓëÓÃ»§Ãû²»Ïà·û£¬ÇëÖØÐÂµÇÂ¼£¡") if ($inpassword ne $password);
}
...
if (((($testentry ne $forumpass)||($testentry eq ""))&&($privateforum eq "yes"))||(($startnewthreads eq "cert")&&($membercode eq "me")&&($userincert eq "no"))) {
	&ERROROUT("Äã²»ÔÊÐí½øÈë¸ÃÂÛÌ³£¡") if(($membercode ne "ad")&&($membercode ne 'smo')&&($inmembmod ne "yes"));
}

&error("½øÈëÂÛÌ³&ÄãµÄÂÛÌ³×éÃ»ÓÐÈ¨ÏÞ½øÈëÂÛÌ³£¡") if ($yxz ne '' && $yxz!~/,$membercode,/);
if ($allowusers ne ''){
    &ERROROUT("Äã²»ÔÊÐí½øÈë¸ÃÂÛÌ³£¡") if (",$allowusers," !~ /,$inmembername,/i && $membercode ne 'ad');
}

if ($membercode ne 'ad' && $membercode ne 'smo' && $inmembmod ne 'yes') {
    &ERROROUT("Äã²»ÔÊÐí½øÈë¸ÃÂÛÌ³£¬ÄãµÄÍþÍûÎª $rating£¬¶ø±¾ÂÛÌ³Ö»ÓÐÍþÍû´óÓÚµÈÓÚ $enterminweiwang µÄ²ÅÄÜ½øÈë£¡") if ($enterminweiwang > 0 && $rating < $enterminweiwang);
	...
	&ERROROUT("Äã²»ÔÊÐí½øÈë¸ÃÂÛÌ³£¬ÄãµÄ½ðÇ®Îª $mymoney£¬¶ø±¾ÂÛÌ³Ö»ÓÐ½ðÇ®´óÓÚµÈÓÚ $enterminmony µÄ²ÅÄÜ½øÈë£¡") if ($enterminmony > 0 && $mymoney < $enterminmony);
	&ERROROUT("Äã²»ÔÊÐí½øÈë¸ÃÂÛÌ³£¬ÄãµÄ»ý·ÖÎª $jifen£¬¶ø±¾ÂÛÌ³Ö»ÓÐ»ý·Ö´óÓÚµÈÓÚ $enterminjf µÄ²ÅÄÜ½øÈë£¡") if ($enterminjf > 0 && $jifen < $enterminjf);
    }
}
...
open(FILE, "$file_of_this_topic") or &ERROROUT("Õâ¸öÖ÷Ìâ²»´æÔÚ£¡¿ÉÄÜÒÑ¾­±»É¾³ý£¡");
@threads = <FILE>;
close(FILE);
...
(my $membername1, my $topictitle1, my $postipaddresstemp1, my $showemoticons1, my $showsignature1, my $postdate1, my $post1, my $posticon1) = split(/\t/, $threads[0]);
	...
	if ($post1 =~m/\[jf=(.+?)\](.+?)\[\/jf\]/isg){ 
	    ...
	    &ERROROUT("ÓÉÓÚÖ÷ÌûÖÐÉèÖÃÓÐ»ý·Ö±êÇ©£¬ËùÒÔÎÞ·¨¿ìËÙ²é¿´£¬ÇëµãÈëºó²é¿´´ËÌû£¬Ð»Ð»£¡") if ($noviewjf eq "yes");
   	    }
   	}
    }
...
#È¡³öÖ÷ÌâÎÄÕÂ
&ERROROUT("±¾Ö÷ÌâÃ»ÓÐÈÎºÎ»Ø¸´¡£") unless(@threads);
...
if ($numberofpages > 1) {
	...
	$startarray = 0;
	$endarray = $numberofitems - 1;
	$pages = "<font color=$menufontcolor>¸ÃÖ÷ÌâµÄ»Ø¸´Ö»ÓÐÒ»Ò³</font>";
}
...
sub OUTPUT_TABLE{
	...
	$OUTPUT_MSG = "(ÎÞÄÚÈÝ)" unless($OUTPUT_MSG ne "");
	#³õÊ¼»¯Êä³öÄÚÈÝ
	my $OUTPUT_TABLE;
	#´¦ÀíÁ¬½áÖµ
	...
	$OUTPUT_TABLE .= qq~<span style=width:100%><span style=width:58%>¡¡¡¡¡¡¡¡$OUTPUT_ICON¡¡<a href=topic.cgi?forum=$inforum&topic=$intopic&start=$START#$SID target=_blank>$OUTPUT_MSG</a></span><span style=width:12%><a href=post.cgi?action=replyquote&forum=$inforum&topic=$intopic&postno=$REPLY_COUNT1 title=ÒýÓÃ»Ø¸´Õâ¸öÌù×Ó target=_blank><img src=$imagesurl/images/replynow.gif border=0 width=16 align=absmiddle></a></span><span style="width:14%">[ <span style="cursor:hand" onClick="javascript:O9('~ . ($uri_escape eq "no" ? $OUTPUT_NAME : uri_escape($OUTPUT_NAME)) . qq~')">$OUTPUT_NAME</span> ]</span><span style=width:14%>$OUTPUT_TIME</span>~;
	#´¦ÀíÊä³öÄÚÈÝ
	$OUTPUT_TABLE =~ s/\n//g;
	$OUTPUT_TABLE =~ s/\'/\\\'/g;
	return "$OUTPUT_TABLE";
